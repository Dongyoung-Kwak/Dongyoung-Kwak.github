...
<body>
	...
	<div class="container">
		...
		<div class="table-responsive">
          ...
          <path
			...
			<table class="table table-hover ">
				...
				<c:forEach var="bDto" items="${list}">
					...
					<tr class="text-center">
						...
						<td scope="col" style="width: 55%;"><c:if
								test="${bDto.re_step > 1 }">
								<c:forEach var="j" begin="1" end="${(bDto.re_step-1)*5}"></c:forEach>
							</c:if> <a
							href="${pageContext.request.contextPath}/board/boardWriteViewCheck.do?num=${bDto.num}">
								${bDto.subject} </a></td>
			...
			<a
				href="${pageContext.request.contextPath}/board/boardListWriteClickAction.do"><button
	...
	<p>
		...
		<a
						href="${pageContext.request.contextPath}/board/boardList.do?pageNum=${startPage-10}">[이전]</a>
				</c:if>
				...
				<c:forEach var="i" begin="${startPage}" end="${endPage}">
					<a
						href="${pageContext.request.contextPath}/board/boardList.do?pageNum=${i}">[${i}]</a>
				...
				<c:if test="${endPage<pageCount}">
					<a
						href="${pageContext.request.contextPath}/board/boardList.do?pageNum=${startPage-10}">[다음]</a>
				</c:if>
			</center>
		</c:if>
